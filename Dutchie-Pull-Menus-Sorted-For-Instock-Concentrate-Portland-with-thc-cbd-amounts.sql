...
WHERE mnp.QuantityAvailable > 0 AND mnp.InsertionDate = date('now', 'localtime') AND pbr.ProductType LIKE '%concentrate%' AND dt.DispensaryCity = 'Portland'
UNION
...
WHERE msp.QuantityAvailable > 0 AND msp.InsertionDate = date('now', 'localtime') AND pb2r.ProductType LIKE '%concentrate%' AND dt2.DispensaryCity = 'Portland'
